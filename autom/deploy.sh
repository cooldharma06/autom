...
$(notify "$(basename "$0"): Deploy the Fuel@MCP MCP stack" 3)
...
Deploys the Fuel@MCP stack on the indicated lab resource.

This script provides the Fuel@MCP deployment abstraction.
It depends on the MCP official configuration directory/file structure
...
-b Base URI to the configuration directory (needs to be provided in URI style,
   ...
   <./mcp/config>.
   The default is using the git submodule tracking 'MCP Pharos' in
   <./mcp/scripts/pharos>.
...
MCP_BONDS=('bond0.2' 'bond0.50' 'bond0.3' 'bond0' 'bond1')
MCP_BRIDGES=('ipmi' 'mgmt' 'ctl')
...
source "${DEPLOY_DIR}/globals.sh"   ## we can use this - dharmalingam s - done
source "${DEPLOY_DIR}/lib.sh"       ## we can use this - dharmalingam s
...
# Clone git submodules and apply our patches                  ###### patches we will remove - cooldharma06 .. :)
...
PHAROS_GEN_CONFIG_SCRIPT="./pharos/config/utils/generate_config.py"                      ### need to modify the yaml based on our env - cooldharma06
...
for tp in "${RECLASS_CLUSTER_DIR}/all-mcp-arch-common/MCP/"*.template \
    net_*.template; do
        eval "cat <<-EOF
		$(<"${tp}")
...
# Determine 'ipmi', 'mgmt', 'ctl' bridge names based on IDF
for ((i = 0; i < ${#BR_NAMES[@]}; i++)); do
    br_jump=$(eval echo "\$parameters__param_MCP_jump_bridge_${BR_NAMES[i]}")
    if [ -n "${br_jump}" ] && [ "${br_jump}" != 'None' ]; then
        MCP_BRIDGES[${i}]="${br_jump}"
    fi
done
notify "[NOTE] Using bridges: ${MCP_BRIDGES[*]}\n" 2
...
if [ ${DRY_RUN} -eq 1 ]; then
    ...
    generate_ssh_key                                                          ##### main function calls - calling mcp/scripts/lib - cooldharma06
    prepare_vms "${base_image}" "${STORAGE_DIR}" "${virtual_repos_pkgs}" \      ## check the base image value
      "${virtual_nodes[@]}"
    verify_networks "${MCP_BRIDGES[@]}"
    create_vms "${STORAGE_DIR}" "${virtual_nodes_data}" "${MCP_BRIDGES[@]}"
	add_ipmi_to_cfg ${                             -----------------------------
